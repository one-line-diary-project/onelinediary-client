...
사실 실무에서는 설정이 이미 다 되어있는 상태에서 작업을 했기 때문에 크게 의식하지 못했던 부분이었습니다만, 처음부터 설정하다 보니 서버와 데이터베이스는 UTC 타임존을 사용하고 있고 이를 단순히 서버 타임존을 한국시간으로 변경하여 해결해서는 안 된다고 생각했습니다.  
사용자 측에서는 서버에 저장된 데이터와 상관없이 본인의 시간대를 기준으로 시간이 표시되어야 하는 게 맞다고 생각했고 이를 클라이언트 측에서 작업하기로 했습니다.
